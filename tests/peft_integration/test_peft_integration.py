...
class PeftIntegrationTester(unittest.TestCase, PeftTesterMixin):
    ...
    def test_peft_load_adapter_applies_user_key_mapping(self):
        ...
        # NOTE(CI-fallback validation): intentionally NOT passing an explicit writable
        # cache_dir, so the download uses the default (read-only in CI) cache and must be
        # rescued by the conftest read-only cache fallback (see conftest._with_tmpdir_cache_fallback).
